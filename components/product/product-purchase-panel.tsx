...
export function ProductPurchasePanel({ product }: { product: Product }) {
  ...
  return <Card className="rounded-3xl p-5 sm:p-7">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2"><Button size="lg" onClick={() => { for (let index = 0; index < quantity; index += 1) add(product); setMessage(`${quantity} ${quantity === 1 ? "item" : "items"} added to cart`); }}><ShoppingBag className="size-4" />Add to cart</Button><Button asChild size="lg" variant="outline"><Link href={`/buy-now/${product.id}`}>Buy now</Link></Button></div>
