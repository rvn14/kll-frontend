...
export default async function AdminItemsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const { items: pageProducts, total: totalItems, pages: apiPages } = await getItemsPaginatedServer({
    page: currentPage,
    limit: ITEMS_PER_PAGE,
  });

  const totalPages = Math.max(1, apiPages);
  const firstItemIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const paginationItems = getPaginationItems(currentPage, totalPages);

  return <div>
    <AdminPageHeader title="Items" description={`${totalItems} items synced from backend.`} actions={<Link href="/admin/items/new" className="inline-flex min-h-11 items-center gap-2 rounded-full bg-brand px-5 text-sm font-black text-white"><Plus className="size-4" />New item</Link>} />
    <section className="panel overflow-hidden">
      ...
      <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-surface-muted text-xs text-ink-muted"><tr><th className="px-4 py-3">Item</th><th className="px-4 py-3">Item ID</th><th className="px-4 py-3">Price</th><th className="px-4 py-3">Stock</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody>{pageProducts.map((product, index) => <tr className="border-t border-border" key={product.id}><td className="px-4 py-3"><div className="flex items-center gap-3"><ProductVisual visual={product.visual} className="size-14 shrink-0" /><span className="max-w-64 font-black text-brand">{product.name}</span></div></td><td className="px-4 py-3 text-xs font-bold text-ink-muted">{product.id}</td><td className="px-4 py-3 font-black text-brand">{displayPrice(product.price)}</td><td className="px-4 py-3 text-ink-muted">{product.stockQuantity ?? "Confirm"}</td><td className="px-4 py-3"><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-black text-emerald-700">Active</span></td><td className="px-4 py-3"><div className="flex justify-end gap-1"><Link href={`/admin/items/${product.id}/edit`} className="flex size-10 items-center justify-center rounded-full text-brand hover:bg-soft/45" aria-label={`Edit ${product.name}`}><Pencil className="size-4" /></Link><button className="flex size-10 items-center justify-center rounded-full text-red-700 hover:bg-red-50" aria-label={`Delete ${product.name}`}><Trash2 className="size-4" /></button></div></td></tr>)}</tbody></table></div>
